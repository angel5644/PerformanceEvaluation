...
namespace PES.Controllers
{
    ...
    public class VacationRequestController : Controller
    {
		...
		public ActionResult HistoricalResource()
		{
			...
			{
				listHeaderReqDB = _headerReqService.GetAllGeneralVacationHeaderReqByManagerId(currentUser.EmployeeId).ToList();
			}
			else if (currentUser.ProfileId == Convert.ToInt32(ProfileUser.Resource))
			{
                listHeaderReqDB = _headerReqService.GetGeneralVacationHeaderReqByEmployeeId(currentUser.EmployeeId).ToList();
			}
            ...
            if (listHeaderReqDB != null && listHeaderReqDB.Count > 0)
            {
                foreach (var headerReq in listHeaderReqDB)
                {
                    var headerReqVm = new VacHeadReqViewModel
                    {
                        ...
                        CreationDate = headerReq.CreationDate,
